...
/* fix application crash due to mem_chunk bug, Luo Zhibiao, 2011-04-01
	       ...
	       gpointer  value)
{
  ...
  tree->root = g_tree_node_insert (tree,
				   ...
				   FALSE, &inserted);
  */
  GTreeNode *new_root = g_tree_node_insert (tree,
                                   tree->root,
				   key, value, 
				   FALSE, &inserted);
  if (new_root)
  ...
  // else create new node failed, remain the tree unchanged
...
#define _GTREE_INSERT_LOG() \
errsv = errno;\
fprintf(stderr, "%s(): %d: create new node failed, %s, "\
          "pid %d, tree 0x%p node 0x%p key 0x%p value 0x%p replace %d\n", \
          __FUNCTION__, __LINE__, strerror(errsv), \
          getpid(), tree, node, key, value, replace)
		    ...
		    gboolean  *inserted)
{
  ...
  GTreeNode *new_node; // fix application crash due to mem_chunk bug, Luo Zhibiao, 2011-04-01

  if (!node)
    {
      new_node = g_tree_node_new (key, value); // if malloc failed, it is NULL
      if (new_node)
      {
        (*inserted) = TRUE;
      }
  ...
  if (cmp < 0)
      ...
      if (node->left)
	{
	  ...
	  if ((old_balance != node->left->balance) && node->left->balance)
	    node->balance -= 1;
	  */
	  new_node = g_tree_node_insert (tree,
                                           node->left,
					   key, value,
					   replace, inserted);
	  if (new_node)
      ...
      else
	{
	  ...
	  *inserted = TRUE;
	  ...
	  node->balance -= 1;
	  */
	  new_node = g_tree_node_new (key, value);
	  if (new_node)
	  {
	    (*inserted) = TRUE;
   	    node->left = new_node;
